...
def perform_comparative_statics_replicated(static_params, sweep_param_name,
                                           sweep_param_values, ai_values,
                                           n_replications=100):
    """
    Replicated comparative statics: run the full analysis multiple times
    to obtain distributions of optimal policy and potential benefit.

    Args:
        static_params (dict): Fixed parameters.
        sweep_param_name (str): Parameter to sweep.
        sweep_param_values (np.ndarray): Values of the swept parameter.
        ai_values (np.ndarray): Candidate 'a' values to optimize over.
        n_replications (int): Number of independent replications.

    Returns:
        dict: Contains per-replication arrays and summary statistics.
    """
    num_sweep = len(sweep_param_values)
    num_ai = len(ai_values)

    # Storage: (n_replications, num_sweep)
    all_optimal_a = np.zeros((n_replications, num_sweep))
    all_potential_benefits = np.zeros((n_replications, num_sweep))

    total_sims = n_replications * num_sweep * num_ai
    print(f"Starting replicated comparative statics for '{sweep_param_name}'")
    print(f"  Replications: {n_replications}, Sweep points: {num_sweep}, "
          f"AI grid: {num_ai}")
    print(f"  Total simulation runs: {total_sims:,}")
    ...
    for r in range(n_replications):
        ...
        for i, sweep_val in enumerate(sweep_param_values):
            dynamic_params[sweep_param_name] = sweep_val
            yields_for_sweep = np.zeros(num_ai)

            for j, ai_prop in enumerate(ai_values):
                dynamic_params['ai_proportion'] = ai_prop
                yields_for_sweep[j] = run_single_simulation(**dynamic_params)

            max_yield = np.max(yields_for_sweep)
            max_index = np.argmax(yields_for_sweep)
            yield_at_ai_1 = yields_for_sweep[-1]

            all_optimal_a[r, i] = ai_values[max_index]
            all_potential_benefits[r, i] = max_yield - yield_at_ai_1

        rep_time = time.time() - rep_start
        elapsed = time.time() - start_time
        eta = elapsed / (r + 1) * (n_replications - r - 1)
        print(f"  Replication {r + 1}/{n_replications} done "
              f"({rep_time:.1f}s, ETA: {eta / 60:.1f} min)")

    total_time = time.time() - start_time
    print(f"All replications complete. Total time: {total_time / 60:.1f} min.")
    ...
    return {
        ...
        "all_optimal_a": all_optimal_a,
        "all_potential_benefits": all_potential_benefits,
        # Summary statistics
        "mean_optimal_a": np.mean(all_optimal_a, axis=0),
        "median_optimal_a": np.median(all_optimal_a, axis=0),
        "ci_lower_optimal_a": np.percentile(all_optimal_a, 2.5, axis=0),
        "ci_upper_optimal_a": np.percentile(all_optimal_a, 97.5, axis=0),
        "mean_potential_benefits": np.mean(all_potential_benefits, axis=0),
        "ci_lower_benefits": np.percentile(all_potential_benefits, 2.5, axis=0),
        "ci_upper_benefits": np.percentile(all_potential_benefits, 97.5, axis=0),
        "static_params": static_params
    }
